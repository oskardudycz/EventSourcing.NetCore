...
and [<NoEquality; NoComparison>] DynamoParameters =
...
and DynamoArguments(c : Configuration, p : ParseResults<DynamoParameters>) =
...
type EsdbParameters =
    ...
    | [<CliPrefix(CliPrefix.None); Unique(*ExactlyOnce is not supported*); Last>] Dynamo of ParseResults<DynamoParameters>
    interface IArgParserTemplate with
        member a.Usage = a |> function
            | Verbose ->                "Include low level Store logging."
...
and EsdbArguments(c : Configuration, a : ParseResults<EsdbParameters>) =
    ...
    member _.CheckpointStoreArgs : CheckpointStoreArguments =
        ...
        | Cosmos cosmos -> CheckpointStoreArguments.Cosmos (CosmosArguments (c, cosmos))
        | Dynamo dynamo -> CheckpointStoreArguments.Dynamo (DynamoArguments (c, dynamo))
...
and [<RequireQualifiedAccess; NoComparison; NoEquality>]
    CheckpointStoreArguments = Cosmos of CosmosArguments | Dynamo of DynamoArguments

[<RequireQualifiedAccess; NoComparison; NoEquality>]
type Store = Cosmos of CosmosArguments | Dynamo of DynamoArguments | Esdb of EsdbArguments
